...
void data(double **E,double **M_sq,int size)
{
    ...
    for(i = 0; i < size; i++)
    {
        ...
        if(N == 2)
            ...
            E_exp = -N * J * (tanh(1/T * J) + pow(tanh(1/T * J),(N - 1))) / (1 + pow(tanh(1/T * J),N));
        
        data[2][i] = E_exp; // Expected E
        data[3][i] = M_sq[i][SAMPLE_SIZE]; // Computed M^2
        
        M_sq_exp = N * (1 + tanh(1/T * J)) / (1 - tanh(1/T * J));
